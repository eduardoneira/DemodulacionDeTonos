...
def plot_windows():
    ...
    plt.title('Comparación ventana rectangular y ventana de Hamming')
    plt.xlabel('Frequency [Hz]')
    plt.ylabel('Magnitude [dB]')

    magnitude, phase, freq = fft(N, rect)
    magnitude_db = 10 * np.log10(magnitude)
    plt.plot(freq, magnitude_db, label='Rectangular')

    magnitude, phase, freq = fft(N, hamming)
    magnitude_db = 10 * np.log10(magnitude)
    plt.plot(freq, magnitude_db, label='Hamming')
